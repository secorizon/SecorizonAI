...
| `SECORIZON_NUM_CTX` | unset (local: `250000`; 1M hosted models: `950000`) | Local Ollama context window or cloud active harness budget. Highest precedence: pinning this at launch overrides the backend default and the `/ctx` runtime command's starting value. Setting it ≤ 32768 also starts the shell in fast mode. |
...
| `SECORIZON_MODEL_BACKEND` | persisted selection, else `local` | Temporary backend override: `local`, `deepseek`, `kimi`, or `kimi-code`. Does not rewrite the saved selection. |
| `SECORIZON_CLOUD_PROVIDER` | persisted provider, else `deepseek` | Optional cloud-provider override: `deepseek`, `kimi`, or `kimi-code`. Setting it also selects that cloud backend unless `SECORIZON_MODEL_BACKEND` is explicit. |
| `SECORIZON_CLOUD_MODEL` | provider default | Temporarily selects the active provider's model (`deepseek-v4-flash`, Open Platform `kimi-k3`, or Kimi Code `k3`) and, unless the backend is explicit, selects that cloud provider. |
...
| `MOONSHOT_BASE_URL` | `https://api.moonshot.ai/v1` | Kimi Chat Completions API base URL. Must be absolute HTTPS. `KIMI_BASE_URL` is accepted as an alias. |
| `MOONSHOT_API_KEY` | saved credential | Temporary Kimi credential override. `KIMI_API_KEY` is accepted as an alias. The key is never added to prompts, history, reports, or model settings. |
| `KIMI_CODE_BASE_URL` | `https://api.kimi.com/coding/v1` | Kimi Code subscription API base URL. This is a separate product and endpoint from the Kimi Open Platform. |
| `KIMI_CODE_API_KEY` | saved credential | Temporary Kimi Code Console credential override. `KIMI_API_KEY` is also accepted when `kimi-code` is explicitly selected. It is stored separately from the Open Platform key because the two key types are not interchangeable. |
| `KIMI_REASONING_EFFORT` | `high` | Kimi K3 reasoning depth: `low`, `high`, or `max`. K3 reasoning itself is always on. `/effort` changes this at runtime; use `max` deliberately when added depth matters more than latency. |

On startup, persisted settings are the default. Explicit `SECORIZON_MODEL` or
`OLLAMA_URL` temporarily selects local Ollama; an explicit backend/provider or
cloud-model variable selects a cloud backend. Environment overrides never rewrite the persisted
...
| `/model [name\|tag]` | Show or persistently change the model on the active backend. Local mode accepts aliases (`v2`, `v3`) or raw Ollama tags and evicts the previous model; cloud mode changes that provider's model name. |
| `/cloudmodel <deepseek\|kimi\|kimi-code> [model]` | Persistently select DeepSeek (default `deepseek-v4-flash`), Kimi Open Platform (default `kimi-k3`), or Kimi Code subscription API (default `k3`) and read that provider's API key with terminal echo disabled. A blank key keeps its separately saved credential. Switching clears conversation context. |
...
| DeepSeek active budget | 950,000 tokens (950K) | `deepSeekPromptBudget` | Default conversation budget within DeepSeek V4's provider-controlled 1M capability. The remaining 50K is reserved for generation. `/ctx` changes the harness budget; it does not send a fictitious Ollama `num_ctx` field to DeepSeek. |
| Kimi K3 active budget | 950,000 tokens (950K) | `kimiPromptBudget` | Default conversation budget within Kimi K3's provider-controlled 1M capability. The remaining 50K is reserved for generation. `/ctx` changes the harness budget; `reasoning_content` is retained as part of assistant history and counts toward that budget. |
| Kimi Code K3 active budget | 950,000 tokens (950K) | `kimiCodePromptBudget` | Same default over the Kimi Code `k3` subscription endpoint when the account grants the advertised up-to-1M service context. Lower membership limits require an explicit smaller `/ctx` or `SECORIZON_NUM_CTX` value. |
