...
| `semanticColorMode` | Opt-in terminal presentation. Buffers only visible response text while showing a live counter, then labels `continue`, `done`, and `question` bodies as reasoning, result, or question. It is session-only and never renders provider-private reasoning content. | `/color [on\|off]`, `--color` |
